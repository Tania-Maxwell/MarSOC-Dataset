...
input_file01 <- "reports/04_data_process/data/data_cleaned_converted.csv"

marshC0 <- read.csv(input_file01)
...
marshC1 <- marshC0 %>% 
  ...
  mutate(Horizon_bin_m = as.factor(case_when(L_depth_m <= 0.1 ~ 0.1,
                                   ...
                                   L_depth_m > 1 ~ NaN))) %>% 
  #calculate stocks
  mutate(OCD_g_cm3 = BD_reported_combined * (OC_perc_combined/100),
...
logDepth_logOC <- openair::scatterPlot(marshC1, x = "Horizon_mid_depth_m", y = "OC_perc_combined", 
                     method = "hexbin", col = "increment", type = "Data_type",
                     log.x = TRUE, log.y = TRUE, xlab="Depth [m]", ylab="OC [%]")
logDepth_logOC

#log OC vs log BS per data type
logOC_logBD <- openair::scatterPlot(marshC1, x = "OC_perc_combined", y = "BD_reported_combined", 
                     method = "hexbin", col = "increment", type = "Data_type",
                     log.x = TRUE, log.y = TRUE, xlab="OC [%]", ylab="BD [g cm-3]")
logOC_logBD

#regular
OC_BD <-openair::scatterPlot(marshC1, x = "OC_perc_combined", y = "BD_reported_combined", 
                     method = "hexbin", col = "increment", type = "Data_type",
...
openair::scatterPlot(marshC1, x = "OC_perc_combined", y = "OCD_kg_m3", 
                     method = "hexbin", col = "increment", type = "Data_type",
...
model_oc_ocd <- glm(I(OCD_kg_m3 + .Machine$double.eps)~log.oc + log.oc2 + 
                      Hoizon_mid_depth_m, data = marshC1,
